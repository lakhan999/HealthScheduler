class DoctorsController < ApplicationController
  ...
  def doctors_appointment_history
    @appointment_history = Appointment.joins(:user).where(doctor_id: @doctor.id).select("appointments.*, users.first_name, users.last_name, users.email, appointments.reason,appointments.status")
  end
